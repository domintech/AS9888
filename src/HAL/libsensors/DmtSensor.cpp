...
#define DMTD_DEFAULT_INTERVAL	200000000
      ...
      mInputReader(32)
{
    ...
    if (data_fd) {
		strcpy(input_sysfs_path, "/sys/class/compass/as9888/");
...
int DmtSensor::setEnable(int32_t handle, int enabled)
{
	...
	case Orientation:
		...
		break;
	default:
		LOGE("DmtSensor: unknown handle (%d)", handle);
		return -EINVAL;
	}
	...
	LOGD("DmtSensor: mEnabled[%d] = %d", id, mEnabled[id]);
...
int DmtSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    if (ns < -1 || 2147483647 < ns) {
		LOGE("DmtSensor: invalid delay (%lld)", ns);
        return -EINVAL;
	}
...
int DmtSensor::handle2id(int32_t handle)
{
    switch (handle) {
        case ID_A:
...
void DmtSensor::processEvent(int code, int value)
{
    switch (code) {
        case EVENT_TYPE_ACCEL_X:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.x = CONVERT_Q16(value);
            break;
        case EVENT_TYPE_ACCEL_Y:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.y = CONVERT_Q16(value);
            break;
        case EVENT_TYPE_ACCEL_Z:
            mPendingMask |= 1<<Accelerometer;
            mPendingEvents[Accelerometer].acceleration.z = CONVERT_Q16(value);
            break;

        case EVENT_TYPE_MAGV_X:
            mPendingMask |= 1<<MagneticField;
            mPendingEvents[MagneticField].magnetic.x = CONVERT_Q16(value);
            break;
        case EVENT_TYPE_MAGV_Y:
            mPendingMask |= 1<<MagneticField;
            mPendingEvents[MagneticField].magnetic.y = CONVERT_Q16(value);
            break;
        case EVENT_TYPE_MAGV_Z:
            mPendingMask |= 1<<MagneticField;
            mPendingEvents[MagneticField].magnetic.z = CONVERT_Q16(value);
            break;
        ...
        case EVENT_TYPE_AZIMUTH:
            mPendingMask |= 1<<Orientation;
            mPendingEvents[Orientation].orientation.azimuth = CONVERT_Q16(value);
            break;
        case EVENT_TYPE_PITCH:
            mPendingMask |= 1<<Orientation;
            mPendingEvents[Orientation].orientation.pitch = CONVERT_Q16(value);
            break;
        case EVENT_TYPE_ROLL:
            mPendingMask |= 1<<Orientation;
            mPendingEvents[Orientation].orientation.roll = CONVERT_Q16(value);
            break;
